...
def get_shipment_by_id(db: Session, shipment_id: int):
    ...
    return db.query(models.Shipment).filter(models.Shipment.id == shipment_id).first() # pragma: no cover
...
def get_organization_by_city(db: Session, city: str):
    return db.query(models.Organization).filter(models.Organization.city == city).first()


def get_contract_by_name(db: Session, Org_name: str):
    return db.query(models.Сontract).filter(models.Сontract.Org_name == Org_name).first()
